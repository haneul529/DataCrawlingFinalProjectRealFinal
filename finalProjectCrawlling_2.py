...
URL = "https://fastcampus.co.kr/category_online_programming"  # 프로그래밍 카테고리 URL
driver.get(URL)
...
for course in courses:
    try:
        ...
        course_data.append({
            ...
            "course_subject": course_subject,  # 세부 과목 추가
            "likes": likes,
            "link": link  # 강의 링크 추가
        })
...
df['course_subject'] = df['course_subject'].fillna('세부 과목 없음')
df['likes'] = df['likes'].fillna('좋아요 수 없음')
df['link'] = df['link'].fillna('링크 정보 없음')

# CSV 파일로 저장 (utf-8-sig 인코딩 사용)
file_name = "fastcampus_courses_programming_with_links.csv"
